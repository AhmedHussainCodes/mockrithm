...
useEffect(() => {
  const fetchUserRole = async () => {
    ...
    try {
      ...
      const userDoc = await getDoc(userDocRef);    // get single document
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      const deletePromises = interviewDocs.docs.map((doc) => deleteDoc(doc.ref));
    ...
    <div className="relative max-w-7xl mx-auto px-4 py-6 space-y-2 z-10">
      ...
      <div className="border-t border-white/20 pt-4 mt-4 space-y-2">
        ...
        {isAdmin && (
          <Link
            href="/admin"
            className="group flex items-center space-x-3 w-full px-4 py-3 rounded-lg text-gray-300 hover:text-white hover:bg-white/10 transition-all duration-200"
            onClick={() => setIsMobileMenuOpen(false)}
          >
        ...
        <button
          ...
          className="group flex items-center space-x-3 w-full px-4 py-3 rounded-lg text-white hover:text-red-300 hover:bg-red-500/10 transition-all duration-300"
        >
